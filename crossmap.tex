...
\begin{abstract} % abstract
  ...
  mistakes, erroneous names, to substitute outdated names wth currently
  available ones. Usually such cheklists are stored in a some sort of a
  spreadsheet, and curators of the list are interested in comparing it with
  either a nomenclatural source like IPNI, MicoBank, or with a
  taxonomical source like Catalogue of Life. Comparing such lists by hand is
  very tedious and error-prone.

  \parttitle{Results}

  We created a convenience tool ``gnCrossmap'' which allows to intelligently
  match checklist entries to an authoritative source and get results back into
  a spreadsheet. The tool is able to process 1 million names in about 10 hours.

  \parttitle{Conclusions}

  We introduce a tool for biodiversity informaticians -- gnCrossmap, which
  greatly simplifies mapping names from an arbitrary cheklist to an
  authoritative source of names.
